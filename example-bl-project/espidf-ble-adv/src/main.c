...
remove_all_bonded_devices();
    ...
    if(esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_N0)) {
        ESP_LOGE(GATTS_TABLE_TAG, "ERROR!!!!!!!!!!!!!");
    }else {
        ESP_LOGE(GATTS_TABLE_TAG, "No-Error???????????");
    }
   
    
    // for (int i = 0; i < ESP_BLE_PWR_TYPE_NUM; i++)
    // {
    //     ret = esp_ble_tx_power_set(i, ESP_PWR_LVL_P1);
    //     if (ret)
    //     {
    //         ESP_LOGE(GATTS_TABLE_TAG, "%i Unable to set BLE Tx power level. err: %x \n", i, ret);
    //         return;
    //     }
    // }
